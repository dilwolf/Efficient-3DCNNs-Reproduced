"""
Kinetics Dataset Cache Builder
Pre-builds dataset_cache.pkl for each split (train, valid) using multiprocessing to scan video directories in parallel.
"""
import os
import pickle
import logging
...
# ---------------------------------------------------------------------------
# Mirrors _get_frame_range() from Kinetics dataloader
# ---------------------------------------------------------------------------
def _get_frame_range(video_dir: Path):
    """Get min and max frame indices by scanning .jpg files in video_dir."""
    min_idx = None
    max_idx = None
...
# Mirrors the inner loop of _build_dataset()
# ---------------------------------------------------------------------------
def scan_class_dir(args):
    """
    Worker function: scans all video directories in one class folder.
    ...
    Returns:
        ...
        samples: list of (video_dir_str, start_idx, end_idx, n_frames, class_idx)
    """
    ...
    try:
        ...
        for video_dir in video_dirs:
            try:
                start_idx, end_idx = _get_frame_range(video_dir)
                n_frames = end_idx - start_idx + 1
                effective_length = sample_duration * sampling_step

                if n_frames < effective_length:
                    continue  # mirrors _process_video_directory() skip logic

                samples.append((str(video_dir), start_idx, end_idx, n_frames, class_idx))

            except IOError:
                continue  # skip empty video dirs silently
...
def build_cache_for_split(
    ...
    # ── Get class directories — mirrors _build_dataset() exactly ────────────
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Pre-build Kinetics dataset cache files")
    parser.add_argument("--root",            type=str, default="/mnt/HDD10TB/kinetics600", help="Root path to Kinetics dataset")
